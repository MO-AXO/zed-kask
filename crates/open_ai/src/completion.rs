...
mod tests {
    ...
    fn test_map_event_populates_cost_from_usage() {
        let usage_with_cost = |cost: Option<f64>, est: Option<f64>, market: Option<f64>| crate::Usage {
        ...
        assert_eq!(
            cost_of(map_completion_events(vec![event_with(usage_with_cost(Some(0.001), None, None))])),
            Some(Some(0.001))
        );
        // DeepInfra: `usage.estimated_cost` (different key, same meaning).
        assert_eq!(
            cost_of(map_completion_events(vec![event_with(usage_with_cost(None, Some(0.002), None))])),
            Some(Some(0.002))
        );
        // KiloCode BYOK: `market_cost` wins over `cost == 0` (real compute energy).
        assert_eq!(
            cost_of(map_completion_events(vec![event_with(usage_with_cost(Some(0.0), None, Some(0.003)))])),
            Some(Some(0.003))
        );
        // Providers that report no cost (Anthropic, Ollama, local) -> `None`.
        assert_eq!(
            cost_of(map_completion_events(vec![event_with(usage_with_cost(None, None, None))])),
            Some(None)
        );
    }
}
