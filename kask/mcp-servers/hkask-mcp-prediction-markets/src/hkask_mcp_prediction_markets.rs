...
mod tests {
    ...
    /// Stub InferencePort for tests — returns an error on any generate call.
    /// EQM scoring tests that need a real LLM are integration tests, not unit tests.
        ...
        > {
            Box::pin(async {
                Err(hkask_types::InferenceError::Generation(
                    "stub: no inference in unit tests".into(),
                ))
            })
        }
    }
